...
export const upvoteComment = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!paramsValidation.success) {
      res.status(400).json(createErrorResponse('Invalid city review comment ID', paramsValidation.error));
      return;
    }
    ...
    if (error instanceof Error) {
      if (error.message === 'City review comment not found') {
        res.status(404).json(createErrorResponse('City review comment not found'));
        return;
      }
    }
    res.status(400).json(createErrorResponse('Failed to upvote city review comment', error instanceof Error ? error.message : 'Unknown error'));
  }
};
...
export const downvoteComment = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(400).json(createErrorResponse('Failed to downvote city review comment', error instanceof Error ? error.message : 'Unknown error'));
  }
};
...
export const updateComment = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.json(createSuccessResponse('City review comment updated successfully', comment));
  } catch (error) {
    if (error instanceof Error) {
      if (error.message === 'City review comment not found') {
        res.status(404).json(createErrorResponse('City review comment not found'));
    ...
    res.status(400).json(createErrorResponse('Failed to update city review comment', error instanceof Error ? error.message : 'Unknown error'));
  }
};
...
export const deleteComment = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.json(createSuccessResponse('City review comment deleted successfully', null));
  } catch (error) {
    if (error instanceof Error) {
      if (error.message === 'City review comment not found') {
        res.status(404).json(createErrorResponse('City review comment not found'));
    ...
    res.status(400).json(createErrorResponse('Failed to delete city review comment', error instanceof Error ? error.message : 'Unknown error'));
  }
}; 